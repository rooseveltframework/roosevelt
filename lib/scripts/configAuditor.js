...
function configAudit (appDir) {
  ...
  // Get the arguments from the npm CLI
  const parsedArgs = JSON.parse(process.env.npm_config_argv)
  const installArgs = [...parsedArgs.cooked]

  // check dependencies
  let checkDeps
  if (installArgs.includes('prod') || installArgs.includes('production')) {
    logger.info('Production install detected. Checking direct dependencies.')
    checkDeps = require('check-dependencies').sync({ packageDir: appDir, scopeList: ['dependencies'] })
  } else {
    logger.info('Checking dependencies and dev dependencies.')
    checkDeps = require('check-dependencies').sync({ packageDir: appDir })
  }
