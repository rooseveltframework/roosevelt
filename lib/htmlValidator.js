    ...
    if (params.htmlValidator.separateProcess.enable && params.htmlValidator.separateProcess.autoKiller) {
      ...
      if (test === true) {
        ...
        fkill(PID, {force: true}).then(() => {
          ...
          let autokiller = spawn('node', [`${path.join(__dirname, 'scripts', 'autoKillValidator.js')}`, `${params.port}`, `${params.htmlValidator.separateProcess.autoKillerTime}`, params.suppressLogs.verboseLogs], {detached: true, stdio: 'inherit', shell: false, windowsHide: true})
          autokiller.unref()
          cb()
        }, () => {
