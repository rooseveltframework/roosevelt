  ...
  userConfigKeys.forEach(key => {
    ...
    switch (key) {
      ...
      case 'clientViews': {
        ...
        const clientViewParam = userConfig[key] || {}
        Object.keys(clientViewParam).forEach(clientViewKey => {
          ...
          switch (clientViewKey) {
            case 'whitelist':
              ...
              break
            case 'blacklist':
              checkTypes(clientViewParam[clientViewKey], clientViewKey, ['null', 'array'])
              required(keyStack, requiredParams)
              break
            case 'output':
