...
module.exports = function (app) {
  ...
  let checkRouters = function (customRouters, directory) {
    ...
    customRouters.filter(router => {
      ...
      if (!Array.isArray(router.files)) {
        errorBool = true
        return false
      } else if (router.files.length === 0) {
        errorBool = true
        return false
      } else {
        return true
    ...
    if (errorBool) {
      logger.warn(`Invalid configuration found in the "routers.${directory}" parameter. Please make sure it is coded correctly. See documentation at http://github.com/kethinov/roosevelt for examples.`.yellow)
    }
  ...
  if (fsr.fileExists(path.normalize(app.get('controllersPath')))) {
    ...
    if (!Array.isArray(routers.controllers)) {
      if (typeof routers.controllers === 'string' && fs.existsSync(path.join(app.get('controllersPath'), routers.controllers))) {
        let schemaLocation = path.join(app.get('controllersPath'), routers.controllers)
      ...
      if (params.alwaysHostPublic || app.get('env') === 'development') {
        if (routers.statics) {
          routers.statics.forEach(statics => {
            ...
            let staticFiles = [...new Set(statics.files)]
            staticFiles.forEach(staticFile => {
              app.use(path.join(statics.prefix, path.extname(staticFile) === '' ? staticFile : ''), express.static(path.join(app.get('publicFolder'), staticFile)))
            })
          })
        } else {
