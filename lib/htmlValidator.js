...
module.exports = function(app, callback) {
      ...
      return new Promise((resolve, reject) => {
        ...
        validatorProcess.stderr.on('data', (data) => {
          if (`${data}`.includes('No Java runtime present, requesting install.')) {
            console.error('There was an error in authentication. Please make sure to have Java installed.');
            reject();
            validatorDisabled = true;
          }
        });
        ...
        validatorProcess.on('error', (err) => {
          if (err.code === 'ENOENT' ) {
            console.error('error: You must install Java to continue. HTML validation disabled, error on initialization (check to make sure Java is installed and in your path)');
          }
          else {
  ...
  setTimeout(function(){console.log('Checking if validator is disabled... ', validatorDisabled);}, 15000);
