...
module.exports = function (app) {
  ...
  if (process.env.HTTP_PROXY || process.env.HTTPS_PROXY) {
    if (process.env.NO_PROXY && !process.env.NO_PROXY.includes('localhost')) {
      params.htmlValidator.enable = false
      logger.warn('system wide proxy environment variable are set, localhost was not as specified as an exception with NO_PROXY')
      logger.warn('The HTML Validator has been disabled')
    }
  }
