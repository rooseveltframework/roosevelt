...
module.exports = function (app, callback) {
  ...
  console.log('RequestHeader In validator', ValidatorExceptions.requestHeader)
  let i
  ...
  function applyValidatorMiddleware () {
    ...
    if (Array.isArray(ValidatorExceptions) && ValidatorExceptions.length >= 1) {
      console.log('my exceptions', ValidatorExceptions[1].requestHeader)
    } // check if there are multiple exceptions

    app.response.render = function (view, model, callback) {
      if (model[ValidatorExceptions.modelValue]) {
        this.req.headers[ValidatorExceptions.requestHeader] = true
      }
      ...
      function multipleHeadersCheck () {
        ...
        if (!res.getHeader(ValidatorExceptions.requestHeader) && (typeof Array.isArray(ValidatorExceptions) !== 'undefined')) {
          ValidatorExceptions.forEach(function (exception) {
            res.set(exception.requestHeader, true)
          })
        } else {
          console.log('NOT going through array')
        }
      }
