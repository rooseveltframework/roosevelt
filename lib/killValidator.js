    ...
    res.on('end', () => {
      try {
        if (rawData.includes('Nu Html Checker')) {
          logger.log('✔️', `Validator successfully found on port: ${validatorOptions.port}`.bold.green)
          foundValidator = true
          killValidator(validatorOptions.port)
        } else {
          logger.warn(`Could not find validator on port: ${validatorOptions.port}. Scanning for validator now...`.bold.yellow)
          scanForValidator()
        }
      } catch (err) {
      ...
      socket.connect(currentPort, host, function () {
        ...
        http.get(options, function (res) {
          ...
          res.on('end', () => {
            try {
              if (rawData.includes('Nu Html Checker')) {
                logger.log('✔️', `Validator succesfully found on port: ${currentPort}`.bold.green)
                foundValidator = true
...
function killValidator (currentPort) {
  ...
  if (process.platform === 'win32') {
    exec('netstat -n -a -o | findstr 0.0.0.0:' + currentPort + '.*LISTENING', function (err, stdout, stderr) {
      ...
      if (!pid) {
        ...
        logger.log('🔍', `Found pid ${pid} on port: ${currentPort}. Killing ${pid}...`.bold.magenta)

        process.kill(pid)
        logger.log('✔️', `Pid ${pid} has been killed`.bold.green)
      }
      logger.log('✔️', `Validator successfully closed on port: ${currentPort}`.bold.green)
      process.exit()
    })
  }
  // for Mac and Ubuntu
  if (process.platform === 'darwin' || process.platform === 'linux') {
    exec('lsof -t -i:' + currentPort + ' | kill -9 $(lsof -t -i:' + currentPort + ')', function (err, stdout, stderr) {
      ...
      logger.log('✔️', `Killed process on port: ${currentPort}`.bold.green)
      process.exit()
    })
  }
}
