...
module.exports = function (app, callback) {
  ...
  function autoKillerStart (cb) {
    ...
    if (params.htmlValidator.separateProcess && !params.noAuto) {
      ...
      if (test === true) {
        ...
        fkill(PID, {force: true}).then(() => {
          ...
          let autokiller = spawn('node', [`${path.join(__dirname, 'scripts', 'autoKillValidator.js')}`, `${params.port}`, `${params.htmlValidator.autoKillerTime}`, params.verboseLogs], {detached: true, stdio: 'inherit', shell: false, windowsHide: true})
          autokiller.unref()
          cb()
        }, () => {
