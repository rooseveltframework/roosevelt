...
module.exports = function (app) {
  ...
  // If the clientViews minifyOptions is empty, default back to the htmlMinify options
  ...
  for (let bundleName in whitelist) {
    ...
    for (let file of bundleFilepaths) {
      ...
      try {
        [bundleLocation, contents] = parseTemplateComment(templateComment, contents, bundleName)
      } catch (error) {
  ...
  if (exposeAll) {
    ...
    for (let filePath of allFilePaths) {
      ...
      let fileName = removeBasePath(filePath, viewsPath)
      ...
      try {
        [bundleLocation, contents] = parseTemplateComment(templateComment, contents, defaultBundle)
      } catch (error) {
        // Error is thrown when template contains <!-- roosevelt-blacklist -->. Skip this template
        logger.error(error)
        continue
