  ...
  if (fsr.fileExists(path.normalize(app.get('controllersPath')))) {
    ...
    if (typeof params.routers === 'string') {
      if (fs.existsSync(path.join(app.get('controllersPath'), params.routers))) {
        ...
        params.routers = require(schemaLocation)
        ...
        if (typeof prefix === 'string' && Array.isArray(router.controllers)) {
          ...
          controllers.forEach((controller, index) => {
            ...
            if (typeof controller === 'string') {
              ...
              if (fs.existsSync(controllerPath)) {
                if (path.extname(controllerPath) === '.js') {
                  ...
                  for (let x = controllerFiles.length - 1; x >= 0; x--) {
                    if (controllerFiles[x].includes(controllerPath)) {
                  ...
                  for (let x = controllerFiles.length - 1; x >= 0; x--) {
                    if (controllerFiles[x].includes(controllerPath)) {
                      requiredController = require(controllerFiles[x])
                      // make sure the controller accepts at least 1 arg
                      if (requiredController.length >= 1) {
                        requiredController(newRouter, app)
                      }
              ...
              logger.warn(`"${util.inspect(controller)}" must be type string but found type ${typeof controller}`.yellow)
            }
          })
        } else {
