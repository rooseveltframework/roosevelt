...
module.exports = function (app) {
  ...
  function getParam (config, keys) {
    const keysArr = [...keys]
    const firstElement = keys !== undefined ? keysArr.shift() : undefined
    const param = firstElement !== undefined ? config[firstElement] : undefined
    if (param === undefined) {
      return undefined
  ...
  function checkParam (paramName, keys) {
    ...
    let param
    const sourceParam = keys === undefined ? params[paramName] : getParam(params, keys)
    const sourcePkg = keys === undefined ? pkg.rooseveltConfig[paramName] : getParam(pkg.rooseveltConfig, keys)
    const sourceDefault = keys === undefined ? defaultConfig[paramName] : getParam(defaultConfig, keys)

    if (sourceParam !== undefined) {
      param = sourceParam
      // logger.log("params: " + param)
    } else if (sourcePkg !== undefined) {
      param = sourcePkg
      // logger.log("pkg: " + param)
    } else if (sourceDefault !== undefined) {
      param = sourceDefault
      // logger.log("default: " + param)
    }
