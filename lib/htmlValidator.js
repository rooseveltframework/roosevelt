...
module.exports = function (app, callback) {
  ...
  let ValidatorExceptions = params.htmlValidator.exceptions
  ...
  console.log('RequestHeader In validator', headerException)
  let i
  ...
  function applyValidatorMiddleware () {
    ...
    app.response.render = function (view, model, callback) {
      ...
      console.log('model exception is', modelException)
      console.log('model exception is', modelException)
    ...
    app.use(tamper((req, res) => {
      ...
      function multipleHeadersCheck () {
        // if (!req.headers[ValidatorExceptions.requestHeader]) {
        //   res.set(ValidatorExceptions.requestHeader, true)
        // } original lines about headers

        console.log('executing multiple header checks')

        if (!res.getHeader(ValidatorExceptions.requestHeader) && (typeof Array.isArray(ValidatorExceptions) !== 'undefined')) {
          ValidatorExceptions.forEach(function (exception) {
            console.log('Looping through validator array 1', exception)
            res.set(headerException, true)
          })
        } else {
          console.log('NOT going through array 1')
        }

        if (Array.isArray(ValidatorExceptions) || Object.getPrototypeOf(ValidatorExceptions) === Object.prototype) {
          if (Object.getPrototypeOf(ValidatorExceptions) === Object.prototype) {
            ValidatorExceptions.forEach(function (exception) {
              // app.response.render = function (view, model, callback) {
              //   if (model[exception.modelValue]) {
              //     this.req.headers[exception.requestHeader] = true
              //   }
              //   render.apply(this, arguments)
              // }

              console.log('Looping through validator array 2', exception)
            })
          } else if (!Array.isArray(ValidatorExceptions)) {
            console.log('going through else if')
          } else {
            console.log('looping through array 2 inside statement')
          }
        } else {
          console.log('NOT going through array 2')
        }
      }

      if (res.statusCode === 200 && res.getHeader('Content-Type') && res.getHeader('Content-Type').includes('text/html') && multipleHeadersCheck()) {
        return body =>
          new Promise(resolve => {
            options.data = body
            options.format = 'json'
