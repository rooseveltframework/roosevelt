...
module.exports = function (app) {
  ...
  if (params.expressSession) {
    ...
    } else if (params.expressSessionStore.preset === 'default' && params.mode === 'production') {
      logger.warn('The `express-session` storage is set to the default "MemoryStore" which works well for single-process apps, but if you\'re spreading your app across multiple processes or servers, you will need to reconfigure `express-session` to use a caching service that supports replication in order to scale your app. More info about that here: https://github.com/rooseveltframework/roosevelt#use-a-caching-service-or-a-database-to-store-sessions')
