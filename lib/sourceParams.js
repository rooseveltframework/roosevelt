...
module.exports = (params, app) => {
  ...
  const config = {
    sources: [
      ...
      { name: 'envVar' },
      { name: 'constructor', source: params },
      { name: 'package', source: pkg.rooseveltConfig }
    ],
    logging: false,
    ...
    transform: (params, flags) => {
      ...
      // handle --production-mode, --prod, -p cli cli flags
      ...
      if (flags.disableValidatorAutokiller || flags.noAutokiller || flags.n) {
        params.htmlValidator.separateProcess.autoKiller = false
      }
  ...
  const schema = {
    ...
    htmlValidator: {
      enable: {
        commandLineArgs: ['--enable-validator', '--html-validator', '-h'],
        default: defaults.htmlValidator.enable
      },
      separateProcess: {
        enable: {
          commandLineArgs: ['--background-validator', '-b'],
          envVar: ['ROOSEVELT_VALIDATOR'],
          default: defaults.htmlValidator.separateProcess.enable
        },
        autoKiller: {
          commandLineArgs: ['--enable-validator-autokiller', '--html-validator--autokiller', '-k'],
          envVar: ['ROOSEVELT_AUTOKILLER'],
    ...
    css: {
      ...
      symlinkSrcToPublic: {
        default: defaults.css.symlinkSrcToPublic
      },
      versionFile: {
    ...
    js: {
      ...
      symlinkSrcToPublic: {
        default: defaults.js.symlinkSrcToPublic
      },
      webpack: {
        enable: {
    ...
    alwaysHostPublic: {
      commandLineArgs: ['--host-public', '--statics', '-s'],
  ...
  params.publicFolder = path.join(appDir, params.publicFolder)
  ...
  if (jsSymlink) {
    // determine status of js bundler
    if (!params.js.webpack || !params.js.webpack.enable) {
  ...
  if (cssSymlink) {
    ...
    if (params.css.compiler === 'none' || !params.css.compiler) {
  ...
  if (app) {
    ...
    app.set('publicFolder', params.publicFolder)
  }
