...
// this fork includes the changes from this PR: https://github.com/attestate/better-sqlite3-session-store/pull/12 plus other new features and fixes we made on top of it
...
module.exports = ({ Store }) => {
  class SqliteStore extends Store {
    constructor (options = {}) {
      ...
      // this is separate from cookie.maxAge on purpose: maxAge decides how long a user stays logged in, this decides how long an abandoned session is kept on disk
    ...
    clearExpiredSessions () {
      try {
        ...
        this.client
          .prepare(`DELETE FROM ${tableName} WHERE lastAccessed IS NOT NULL AND lastAccessed < @cutoff`)
          .run({ cutoff: Date.now() - this.maxInactivity })
      } catch (err) {
        console.error(err)
      }
    }
    ...
    createDb () {
      ...
      // update session store with new schema if using a version that existed prior to the addition of the lastAccess feature
      const columns = this.client.prepare(`PRAGMA table_info(${tableName})`).all()
      if (!columns.some(column => column.name === 'lastAccessed')) {
        this.client.exec(`ALTER TABLE ${tableName} ADD COLUMN lastAccessed INTEGER`)
        this.client.prepare(`UPDATE ${tableName} SET lastAccessed = ?`).run(Date.now())
      }
    }
    ...
    set (sid, sess, cb = noop) {
      ...
      const entry = { sid, sess: JSON.stringify(sess), expire, lastAccessed: now }
    ...
    touch (sid, sess, cb = noop) {
      const entry = { sid, lastAccessed: new Date().getTime() }
      ...
      try {
        res = this.client
          .prepare(`
            UPDATE ${tableName}
            SET expire = @expire, lastAccessed = @lastAccessed
            WHERE sid = @sid AND datetime('now') < datetime(expire)
          `)
          .run(entry)
      } catch (err) {
