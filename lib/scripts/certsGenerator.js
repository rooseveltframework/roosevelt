...
function certsGenerator (appDir) {
  ...
  var newPkcs12Asn1 = forge.pkcs12.toPkcs12Asn1(privateKey, certificate, '', { generateLocalKeyId: true, algorithm: '3des' })
  ...
  try {
    ...
    fs.writeFile('./certs/cert.p12', newPkcs12Der, err => {
      if (err) {
        console.error(err)
      }
      // file written successfully
    })
    // fs.writeFile('./certs/cert.p12', pkcsAsn1.toString())
  } catch {}
}
