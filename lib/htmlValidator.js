...
module.exports = function(app, callback) {
  ...
  if (!params.enableValidator) {
    ...
    if (params.htmlValidator.validatorAlwaysOn) {
      // if validatorAlwaysOn is enabled, spawn a detached validator
      promise = (() => {
        return new Promise((resolve, reject) => {
          validatorProcess = spawn(
            'java', ['-Xss1024k', '-cp', vnu, 'nu.validator.servlet.Main', params.htmlValidator.port ? params.htmlValidator.port : '8888'], {detached: true}
          );
      ...
      // if validatorAlwaysOn is disabled, spawn a validator attached to node process
      promise = (() => {
        return new Promise((resolve, reject) => {
          validatorProcess = spawn(
