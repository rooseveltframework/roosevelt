...
module.exports = function (app) {
  ...
  const logger = require('roosevelt-logger')(app.get('params').logging)
  ...
  let checkRouters = function (customRouters, directory) {
    let errorBool = false
    let filesOrDirs = directory === 'controllers' ? 'files' : 'dirs'
    customRouters = customRouters.filter(router => {
      ...
      router[filesOrDirs] = router[filesOrDirs].filter(file => {
        ...
        if (!fs.existsSync(path.join(app.get(directory === 'controllers' ? 'controllersPath' : 'publicFolder'), file))) {
          logger.warn(`${app.get('appName')} failed to load the ${path.extname(file) === '.js' ? 'file' : 'directory'}: "${file}" to use with the router associated with ${directory === 'controllers' ? 'controller' : 'public'} prefix: ${router.prefix}`.yellow)
          return false
        }
        return true
      })
    ...
    if (controllerFiles) {
      ...
      controllerFiles.forEach(controller => {
        ...
        if (controller !== app.get('params').errorPages.notFound) {
          ...
          let exceptions = ['.DS_Store'] // do not attempt to execute controller logic on these files
