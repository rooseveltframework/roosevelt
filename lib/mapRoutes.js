...
module.exports = function (app) {
  ...
  // set the url prefix
  urlPrefix = validatePrefix(params.urlPrefix)

  // map statics for developer mode
  if (params.alwaysHostPublic || app.get('env') === 'development') {
    app.use(urlPrefix, express.static(app.get('publicFolder')))
  }
  ...
  if (fsr.fileExists(path.normalize(app.get('controllersPath')))) {
    ...
    if (urlPrefix !== '/') router = app.get('express').Router()
    ...
    // if a router has been setup, pass that to the app middleware stack
    if (router) {
