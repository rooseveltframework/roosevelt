...
function certsGenerator (appDir, keyFolder) {
  ...
  if (params.https?.authInfoPath?.authCertAndKey) {
    ...
    if (!fs.existsSync(path.join(filepath, params.https.authInfoPath.authCertAndKey.key))) fs.writeFileSync(path.join(filepath, params.https.authInfoPath.authCertAndKey.key), key)
    if (!fs.existsSync(path.join(filepath, params.https.authInfoPath.authCertAndKey.cert))) fs.writeFileSync(path.join(filepath, params.https.authInfoPath.authCertAndKey.cert), cert)
  }
}
