...
module.exports = function (app, callback) {
  ...
  function applyValidatorMiddleware () {
    ...
    app.response.render = function (view, model, callback) {
      ...
      let requestHeaders = this.req.headers
      if (modelException) {
        if (typeof modelException === 'string') {
          if (model[modelException]) {
            applyHeaderExceptions()
          }
        } else if (Array.isArray(modelException)) {
          let found = false
      ...
      function applyHeaderExceptions () {
        if (typeof headerException === 'string') {
          requestHeaders[headerException.toLocaleLowerCase()] = true
        } else if (Array.isArray(headerException)) {
          headerException.forEach(function (exception) {
            requestHeaders[exception.toLocaleLowerCase()] = true
          })
        }
      }
    ...
    app.use(tamper((req, res) => {
      ...
      function validatorExceptions () {
        if (typeof headerException === 'string') {
          if (req.headers[headerException.toLowerCase()]) {
