...
module.exports = async app => {
  ...
  // TODO: this needs to become async once roosevelt.js is refactored accordingly
  const allViewsFiles = klawSync(viewsPath, { filter: item => !finalBlocklist.has(item.path), nodir: true }).map(file => file.path)
  ...
  for (const bundleName of Object.keys(bundles)) {
    ...
    for (const file of bundleFiles) {
      ...
      let templateContent = fs.readFileSync(fullPath, 'utf8').trim()
