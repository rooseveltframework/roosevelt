...
module.exports = function(app, callback) {
  ...
  if (preprocessor === 'roosevelt-closure') {
    ...
    // get roosevelt-closure dependency version
    preprocessorDep = package.dependencies[preprocessor];

    // create exception for version set to file path
    if (!preprocessorDep.startsWith('.')) {
      ...
      if (preprocessorVersion < 40) {
        console.error('❌  Incompatible version of module roosevelt-closure detected. Please update to at least version 0.4.0'.bold.red);
        process.exit();
      }
    }
  }
