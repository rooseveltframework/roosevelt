...
module.exports = app => {
  ...
  //
  // this is the same list the static page generator already skips: roosevelt's own, plus whatever the app's .gitignore names, which is what keeps the .DS_Store macos drops into a folder from starting a rebuild
  ...
  //
  // closing the watchers stops new edits being noticed, but not a rebuild already waiting out its debounce, so that is cancelled too, and one already under way does nothing more once it gets there. a rebuild that outlived its app raced whatever ran next in the same process: the static page generator changes the working directory while it renders, so two of them at once could each put back the other's, and leave the process in a folder that was about to be deleted
  let stopped = false
  watchers.push({
    close () {
      stopped = true
  ...
  async function rebuild () {
    if (stopped || rebuilding || !edited.size) return
    rebuilding = true
    const files = [...edited]
    ...
    try {
      ...
      // the static pages may have changed, so the sitemap lists them again
