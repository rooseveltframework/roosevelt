...
module.exports = function (app, callback) {
  ...
  console.log('I AM SIN THE VALIDATOR PHASE')
  console.log('RequestHeader', ValidatorExceptions.requestHeader)
  let i
  ...
  function applyValidatorMiddleware () {
    ...
    if (Array.isArray(ValidatorExceptions) && ValidatorExceptions.length >= 1) {
      console.log('I am an array and the exceptions are multiple'.toLocaleUpperCase())
    } else {
      console.log('I AM NOT AN ARRAY WITH MULTIPLE EXCEPTIONS', ValidatorExceptions)
    }
    ...
    app.use(tamper((req, res) => {
      ...
      if (res.statusCode === 200 && res.getHeader('Content-Type') && res.getHeader('Content-Type').includes('text/html') && !res.getHeader(ValidatorExceptions.requestHeader) && multipleHeadersCheck) {
        return body =>
          new Promise((resolve) => {
            options.data = body
            options.format = 'json'
