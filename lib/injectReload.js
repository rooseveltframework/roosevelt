...
module.exports = app => {
  ...
  if (app.get('env') === 'development' && reloadParams.enable) {
    app.use(require('./tools/captureResponseBody')((req, res) => {
      if (!wildcardMatch(req.url, reloadParams.exceptionRoutes) && res.getHeader('Content-Type') && res.getHeader('Content-Type').includes('text/html')) {
        return (body) => {
          ...
          if (pos > 0) body = body.substring(0, pos) + `<script src="${reloadParams?.expressBrowserReloadParams?.route ? reloadParams?.expressBrowserReloadParams?.route : '/express-browser-reload.js'}"></script> <!-- Injected by Roosevelt for frontend reload functionality -->\n</body>` + body.substring(pos + 7)
          return body
        }
      }
    }))
