...
module.exports = function (app, callback) {
  ...
  if (!fsr.fileExists(jsPath)) {
    if (fsr.ensureDirSync(jsPath)) {
      logger.info('📁', `${appName} making new directory ${jsPath}`.yellow)
    }
  }
  ...
  if (params.minify) {
    ...
    if (jsCompiler && typeof jsCompiler.parse === 'function') {
      ...
      logger.info('⚙️', `${appName} using your custom JS preprocessor`.bold)
    } else {
      try {
        // require preprocessor via a dependency
        preprocessorModule = prequire(preprocessor)
      } catch (err) {
        logger.error(`${appName} failed to include your JS compiler! Please ensure that it is declared properly in your package.json and that it has been properly installed to node_modules.`)
        logger.error(err)
        logger.warn('JS compiler has been disabled')
    ...
    if (typeof preprocessorModule.parse === 'function') {
      ...
      if ((preprocessorArgs.length !== 2) || (preprocessorArgs[0] !== 'app') || (preprocessorArgs[1] !== 'fileName')) {
        logger.error(`Selected JS compiler module ${preprocessor} out of date or incompatible with this version of Roosevelt.`.bold)
        process.exit(1)
      }
    } else {
      logger.error(`Selected JS compiler module ${preprocessor} out of date or incompatible with this version of Roosevelt.`.bold)
  ...
  if (!fsr.fileExists(jsCompiledOutput)) {
    if (fsr.ensureDirSync(jsCompiledOutput)) {
      logger.info('📁', `${appName} making new directory ${jsCompiledOutput}`.yellow)
    }
  }
  ...
  if (usingWhitelist) {
    if (typeof params.js.whitelist !== 'object') {
      logger.error('JS whitelist not configured correctly. Please ensure that it is an array. See https://github.com/rooseveltframework/roosevelt#statics-parameters for configuration instructions')
      callback()
      return
    } else {
  ...
  jsFiles.forEach((file) => {
    ...
    promises.push(
      new Promise((resolve, reject) => {
        ...
        const newFile = path.join(jsCompiledOutput, (altdest || file))

        // disable minify if minify param is false
        if (!params.minify || blacklist.includes(file.replace(/^[\\/]+/, ''))) {
          newJS = fse.readFileSync(path.join(jsPath, file), 'utf-8')
        } else {
          // compress the js via the compiler set in roosevelt params
          try {
            newJS = preprocessorModule.parse(app, file)
          } catch (err) {
            logger.error(`${appName} failed to parse ${path.join(jsPath, file)}. Please ensure that it is coded correctly`)
            reject(err)
            return
          }
        }
        ...
        if (fse.readFileSync(newFile, 'utf8') !== newJS) {
          if (fsr.writeFileSync(newFile, newJS)) {
            logger.info('📝', `${appName} writing new JS file ${newFile}`.green)
          }
          resolve()
        } else {
          resolve()
