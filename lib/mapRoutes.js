...
module.exports = function (app) {
  ...
  if (params.alwaysHostPublic || app.get('env') === 'development') {
    if (params.urlPrefix) {
      app.use(params.urlPrefix, express.static(app.get('publicFolder')))
    } else {
  ...
  if (fsr.fileExists(path.normalize(app.get('controllersPath')))) {
    ...
    if (params.urlPrefix) router = app.get('express').Router()
    controllerFiles.forEach((controllerName, index) => {
      let controller
        ...
        try {
          if (fs.statSync(controllerName).isFile() && path.extname(controllerName) === '.js') {
            ...
            if (controller.length === 1) {
              if (params.urlPrefix) {
                // pass the router to the controller
                controller(router)
              } else {
                controller(app)
              }
    ...
    if (router) {
      app.use(params.urlPrefix, router)
    }
  }
