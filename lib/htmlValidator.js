...
module.exports = function (app, callback) {
  ...
  function applyValidatorMiddleware () {
    ...
    app.response.render = function (view, model, callback) {
      if (model) {
        if (Array.isArray(ValidatorExceptions.requestHeader)) {
          if (ValidatorExceptions.includes(ValidatorExceptions.requestHeader)) {
    ...
    app.use(tamper((req, res) => {
      ...
      function multipleHeadersCheck () {
        if (!res.getHeader(ValidatorExceptions.requestHeader)) {
          if (Array.isArray(ValidatorExceptions.requestHeader) && ValidatorExceptions.includes(ValidatorExceptions.requestHeader)) {
            res.set(ValidatorExceptions.requestHeader, true)
          } else {
            return printValidatorErrors
          }
        } else {
          res.set(ValidatorExceptions.requestHeader, true)
        }
      ...
      function printValidatorErrors (body) {
