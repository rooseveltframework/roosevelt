...
//
// this is separate from the main test suite because it hits the network and takes about a minute, but it catches a class of problem the suite structurally cannot: the suite runs against this repo, where every dependency is already present, so it can never notice a file missing from the published package or a peer dependency that the app has to supply for itself
...
`)
  ...
  fs.writeFileSync(path.join(appDir, 'roosevelt.config.js'), `const rooseveltConfig = require('roosevelt/config')
module.exports = {
...
`)
  ...
  fs.rmSync(path.join(appDir, 'roosevelt.config.js'), { force: true })
