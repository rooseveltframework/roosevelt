...
module.exports = (options, params) => {
  ...
  try {
    dependencies = fs.readJsonSync(path.join(params.appDir, 'package.json')).dependencies || {}
  ...
  let cfg = {}
  for (const name of ['rooseveltConfig.js', 'roosevelt.config.js']) {
    try {
  ...
  const checks = [
    ...
    [() => fs.existsSync(path.join(params.appDir, 'rooseveltConfig.json')) || fs.existsSync(path.join(params.appDir, 'roosevelt.config.json')), () => {
      ...
      const jsConfig = ['rooseveltConfig.js', 'roosevelt.config.js'].find(name => fs.existsSync(path.join(params.appDir, name)))
      ...
      return 'Your config is still a JSON file, which Roosevelt no longer reads. Run `npx roosevelt-migrate-config` in your app directory to convert it to `rooseveltConfig.js`, then delete the JSON file.'
    }],
    [() => {
    ...
    }, 'Your `package.json` has a `rooseveltConfig` key, which Roosevelt no longer reads, so those params are not being applied. Run `npx roosevelt-migrate-config` in your app directory to convert it to `rooseveltConfig.js`, then delete the key.'],
    [() => {
