  ...
  for (const bundleName in allowlist) {
    ...
    for (let file of bundleFilepaths) {
      if (!file.includes('*')) {
        ...
        fs.readdirSync(fileNameAndPath).forEach(content => {
          const contentNameAndPath = path.join(fileNameAndPath, content)

          try {
            if (fse.lstatSync(contentNameAndPath).isDirectory()) {
              exposeDirectory(contentNameAndPath, bundleName, 'allowlist')
            } else {
              contents = fse.readFileSync(contentNameAndPath, 'utf8').trim()
              filesCache.push(contentNameAndPath)
            }
          } catch (error) {
            // File may not exist. Log such error
            logger.error(error)
  ...
  function saveBundle (bundle, filename) {
    ...
    for (const tempFilename in bundle) {
      ...
      if (tempFilename.substring(tempFilename.length - app.get('view engine').length - 1, tempFilename.length) === '.' + app.get('view engine')) {
        ...
        newBundle[bundleFileName] = preprocess(bundle[tempFilename], tempFilename)
      } else {
        bundleFileName = tempFilename
        newBundle[bundleFileName] = preprocess(bundle[tempFilename], tempFilename)
      }
    }
