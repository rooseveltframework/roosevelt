...
module.exports = async app => {
  ...
  const allViewsFiles = (await walk(viewsPath)).filter(file => !finalBlocklist.has(file))
  ...
  for (const bundleName of Object.keys(bundles)) {
    try {
      ...
      const bundleFiles = matchingFiles(bundleGlob, viewsPath, finalBlocklist)
      const bundleDataStructure = {}

      const writePath = path.join(app.get('clientViewsBundledOutput'), bundleName)

      // write views bundle only if it contains new/edited files
      const bundleSources = bundleFiles.map(file => path.join(viewsPath, file))
