...
const validatorErrorPage = fse.readFileSync(path.join(__dirname, '../defaultErrorPages/views/htmlValidator.html'))
const os = require('os')
const fkill = require('fkill')
const Prism = require('prismjs')
const prismPath = require.resolve('prismjs')
const prismStyleSheet = fse.readFileSync(path.join(prismPath.split('prism.js')[0], 'themes/prism.css'))
...
module.exports = function (app, callback) {
  ...
  function autoKillerStart (cb) {
    ...
    if (params.htmlValidator.separateProcess.enable && params.htmlValidator.separateProcess.autoKiller) {
      ...
      const test = fse.existsSync(PIDPath)
      if (test === true) {
