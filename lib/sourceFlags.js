...
module.exports = function (ignoreCLIFlags) {
  ...
  commandLineArgs.forEach((arg, index, array) => {
    switch (arg) {
      ...
      case '-dev':
        logger.warn(`Detected use of "${arg}" command line argument. This will soon be deprecated in favor of "--development-mode" / "--dev" / "-d"`.yellow)
        checkFlags('developmentMode', 'productionMode')
        break
      case '-prod':
        logger.warn(`Detected use of "${arg}" command line argument. This will soon be deprecated in favor of "--production-mode" / "--prod" / "-p"`.yellow)
        checkFlags('productionMode', 'developmentMode')
        break
      case '-cores':
        logger.warn(`Detected use of "${arg}" command line argument. This will soon be deprecated in favor of "--cores" / "-c"`.yellow)
        flags.cores = array[index + 1]
        break
      case 'enable-validator':
        logger.warn(`Detected use of "${arg}" command line argument. This will soon be deprecated in favor of "--enable-validator" / "--html-validator" / "-h"`.yellow)
        checkFlags('enableValidator', 'disableValidator')
        break
      case 'disable-validator':
        logger.warn(`Detected use of "${arg}" command line argument. This will soon be deprecated in favor of "--disable-validator" / "--raw" / "-r"`.yellow)
        checkFlags('disableValidator', 'enableValidator')
        break
      case 'detach-validator':
        logger.warn(`Detected use of "${arg}" command line argument. This will soon be deprecated in favor of "--background-validator" / -b"`.yellow)
        checkFlags('backgroundValidator', 'attachValidator')
        break
      case 'attach-validator':
        logger.warn(`Detected use of "${arg}" command line argument. This will soon be deprecated in favor of "--attach-validator" / "-a"`.yellow)
