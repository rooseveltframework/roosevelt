...
module.exports = function (app, callback) {
  ...
  if (fsr.fileExists(path.join(app.get('appDir'), '.gitignore'))) {
    ignoreExists = true
  } else {
    logger.log('🔔', 'No .gitignore file found. Compiler will get files to ignore from list of defaults.'.yellow)
    defaults = ['lib-cov', 'node_modules', 'pids', 'logs', 'results', 'Thumbs.db', '.seed', 'npm-debug.log', '.build', 'public', '.log', '.csv', '.dat', '.out', '.pid', '.gz', '.DS_Store']
  }
  ...
  jsFiles.forEach((file) => {
    ...
    promises.push(
      new Promise((resolve, reject) => {
        ...
        if (ignoreExists) {
          if (dotgit.ignore(file) || file === '.' || file === '..') {
            logger.log('💭', `${file} read from .gitignore and will not be compiled`)
            resolve()
          }
        } else {
          let length = defaults.length
          // scan through list of defaults for file
          while (length--) {
            if (file === '.' || file === '..' || file.indexOf(defaults[length]) !== -1 || fs.lstatSync(usingWhitelist ? path.join(jsPath, file) : file).isDirectory()) {
              logger.log('💭', `${file} read from defaults and will not be compiled`)
              resolve()
            }
          }
        }
        ...
        if (params.noMinify || blacklist.includes(file.replace(/^[\\/]+/, ''))) {
