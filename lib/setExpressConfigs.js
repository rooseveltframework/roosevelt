...
module.exports = app => {
  ...
  if (params.csrfProtection && params.makeBuildArtifacts !== 'staticsOnly') {
    ...
    const csrfUtilities = doubleCsrf({
      ...
      getSessionIdentifier: (req) => req.session?.id ? req.session?.id : '', // method to retrieve the session id
    ...
    app.use((req, res, next) => {
      ...
      res.csrfToken = csrfUtilities.generateCsrfToken(req, res, { validateOnReuse: true })
      next()
    })
  } else {
