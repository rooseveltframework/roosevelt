...
module.exports = function (app, callback) {
  ...
  const preprocessor = params.css.compiler.nodeModule
  ...
  if (params.css.versionFile) {
    if (!params.css.versionFile.fileName || typeof params.css.versionFile.fileName !== 'string') {
      ...
      if (fse.readFileSync(versionFile, 'utf8') !== versionCode) {
  ...
  // clean-css options
  const opts = params.css.compiler.params.cleanCSS || {}
  ...
  cssFiles.forEach((file) => {
    ...
    promises.push(
      new Promise((resolve, reject) => {
        ...
        if (file === '.' || file === '..' || file === 'Thumbs.db' || fse.lstatSync(usingWhitelist ? path.join(cssPath, file) : file).isDirectory()) {
          resolve()
          return
        }
        ...
        preprocessorModule.parse(app, file)
          .then(([newFile, newCSS]) => {
            ...
            if (fse.readFileSync(newFile, 'utf8') !== newCSS) {
