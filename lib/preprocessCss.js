...
const requireFromApp = require('./tools/requireFromApp')
...
module.exports = async app => {
  ...
  if (!customPreprocessor) {
    ...
    try {
      module = requireFromApp(app.get('appDir'), moduleName, require) // the preprocessor is the app's to install, so it is loaded from the app
    } catch (err) {
    ...
    // the places a preprocessor looks for imported files: the app's css folder first, then any the app supplied itself, rather than the css folder replacing them. less also accepts a single path as a string, so one is taken either way
    const withCssPath = (app, supplied) => [app.get('cssPath'), ...[].concat(supplied || []).filter(dir => dir !== app.get('cssPath'))]
      ...
      preprocessorModule = {
        ...
        parse: (app, file) => {
          return new Promise((resolve, reject) => {
            const userOptions = params.css.compiler.options || {} // an app can set this to null, and the import paths are read from it below
            const options = {
              ...userOptions,
              loadPaths: withCssPath(app, userOptions.loadPaths),
            ...
            let { css, sourceMap, loadedUrls } = module.compile(file, options)
            ...
            resolve({
              css,
              sourceMap,
              deps: loadedUrls // the files the build cache watches for edits; sass includes this file along with its imports
            })
          })
        }
      }
      ...
      preprocessorModule = {
        ...
        parse: (app, file) => {
          return new Promise((resolve, reject) => {
            ...
            const options = {
              ...userOptions,
              paths: withCssPath(app, userOptions.paths),
            ...
            module.render(fs.readFileSync(file, 'utf8'), options, (err, output) => {
              ...
              resolve({
                ...
                deps: [file, ...(output.imports || [])] // the files the build cache watches for edits; less lists only the imports, so this file is added too
              })
            })
          })
        }
        ...
        parse: (app, file) => {
          return new Promise((resolve, reject) => {
            ...
            if (app.get('env') === 'development' || params.prodSourceMaps) {
              style.set('sourcemap', {
                comment: false,
                inline: true // we should set this to false if `sourcesContent` is ever implemented https://github.com/stylus/stylus/issues/2036 — inline source maps are required in stylus because Roosevelt does not expose source files and `sourcesContent` is not implemented
              })
            }
            ...
            style.render(function (err, css) {
              ...
              // convert source map absolute paths to relative paths
              //
              // once `sourcesContent` is implemented https://github.com/stylus/stylus/issues/2036 we uncomment this block
              // if (style.sourcemap) {
              ...
              resolve({
                ...
                deps: [file, ...(typeof style.deps === 'function' ? style.deps() : [])] // the files the build cache watches for edits; stylus lists only the imports, so this file is added too
              })
            })
          })
        }
  ...
  if (usingAllowlist) {
    if (typeof params.css.allowlist !== 'object') {
      logger.error(`CSS allowlist not configured correctly. Please ensure that it is an array. See ${docsUrl}/config-css for configuration instructions`)
      return
        ...
        new Promise((resolve, reject) => {
          (async () => {
            ...
            try {
              ...
              // compile this file only if it or one of its imports was edited
              if (buildCache.isFresh(outpath)) return resolve()
              ...
              if (newCSS !== '' && (!content || content !== newCSS)) {
                ...
                if (newSourceMap) {
                  ...
                  outputs.push(sourceMapOutpath)
                }
              } else if (newSourceMap && fs.pathExistsSync(outpath.endsWith('.css') ? outpath.slice(0, -4) + '.map' : outpath)) {
                // the css was unchanged so nothing was rewritten, but the existing source map is still part of this artifact and must be watched for deletion
                outputs.push(outpath.endsWith('.css') ? outpath.slice(0, -4) + '.map' : outpath)
              }

              // remember this file and its imports for the next build, unless the preprocessor did not say what the imports were
              if (cssObj.deps) buildCache.record(outpath, { outputs, sources: cssObj.deps })
              resolve()
            } catch (e) {
              // this file failed to compile, so compile it again next time
              buildCache.forget(path.join(cssCompiledOutput, altdest || path.join(path.parse(file).dir.replace(cssPath, ''), `${path.parse(file).name}.css`)))
