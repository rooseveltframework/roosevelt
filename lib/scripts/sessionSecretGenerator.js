...
// the app is the directory the command was run from, which is where npx and npm scripts run it. this used to be worked out as four directories up from this file, which is the app only when roosevelt sits in the app's node_modules, and a roosevelt linked to a clone is not there
const runFrom = process.cwd()

// source from the app's config file
for (const name of ['roosevelt.config.js', 'rooseveltConfig.js']) {
  if (appDir) break
  try {
    const rooseveltConfig = require(path.join(runFrom, name))
