    ...
    for (let file of await walk(htmlPath)) {
      ...
      if (file !== '.' && file !== '..' && !gitignoreFiles.includes(path.basename(file)) && !gitignoreFiles.includes(file) && !fs.lstatSync(file).isDirectory()) {
        try {
          ...
          } else outpath = path.join(htmlRenderedOutput, dir.replace(htmlPath, ''), `${name}.html`)
          ...
          if (newHtml !== '' && content !== newHtml) {
            ...
            if (!validHtml) logger.error(`↳ The file has HTML validation errors. Upload ${outpath} to https://validator.w3.org for details.`)
          }
          continue
        } catch (e) {
