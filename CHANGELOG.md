...
- Fixed the import paths supplied in `css.compiler.options` being replaced by the app's CSS folder, which is `loadPaths` for Sass and `paths` for Less and Stylus. The CSS folder is now added to them instead.
- Fixed Roosevelt loading its own copy of a package the app is supposed to supply, instead of the app's, whenever Roosevelt has a copy of its own. That covers Express, the view engine, the CSS preprocessor, and the JS bundler. An ordinary install was unaffected, but a Roosevelt linked to a clone, copied in by `devSync.js`, or installed by a package manager that keeps each package's dependencies apart, such as pnpm, would quietly run its development copies rather than the versions the app installed. They are now loaded from the app, with Roosevelt's own copy as the fallback.
- Fixed a failed render being sent to the visitor as JSON, stack trace included, instead of going to the app's error page, whenever HTML minification was enabled, which it is by default in production. A failed render with a callback also no longer tries to minify HTML that was never produced, and `res.render(view, callback)` with no options is now minified like any other render.
- Fixed `roosevelt-generate-certs` and `roosevelt-generate-session-secret` looking for the app four directories up from Roosevelt's own files when not given `--appDir`. They now use the directory the command was run from, the same as `roosevelt-migrate-config`.
