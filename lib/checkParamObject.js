  ...
  if (mode === 'setDefault') {
    defaultKeys.forEach((key) => {
      ...
      if (!defaultProp.optional) {
        if (defaultProp.value instanceof Object && !(Array.isArray(defaultProp.value))) {
    ...
    if (userKeys.length < defaultKeys.length) {
      defaultKeys.forEach((key) => {
        ...
        if (!defaultProp.optional) {
          if (!(key in userParam)) {
            if (mode === 'audit') {
              console.error('⚠️ ', `Missing param ${key} in ${defaultProp.parent}!`.red.bold)
            } else if (defaultProp.value instanceof Object && !(Array.isArray(defaultProp.value))) {
