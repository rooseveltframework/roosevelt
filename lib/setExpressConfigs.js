...
module.exports = function (app) {
  ...
  if (params.expressSession) {
    ...
    const {
      ...
      // used in routes to provide a CSRF hash + token cookie and token.
      generateToken,

      // the default CSRF protection middleware.
      doubleCsrfProtection
    } = doubleCsrf({
      // name of the httpOnly cookie used to track CSRF protection - default is "__Host-psifi.x-csrf-token"
      // https://github.com/Psifi-Solutions/csrf-csrf#cookiename
      // ? cookieName: '_csrf',

      // options provided to the cookie, can also include cookie attributes
      cookieOptions: {
        // sameSite: 'lax', // default, they recommend strict if possible
        secure: true // default
      },

      // provides a secret key to be used for hashing the CSRF tokens
      getSecret: () => 'secret',
      // getSecret: () => secret,

      // method to retrieve token by the doubleCsrfProtection middleware
      getTokenFromRequest: (req) => req.csrfToken()
    })

    function csrfErrorHandler (error, req, res, next) {
      if (error === invalidCsrfTokenError) {
        // * this serves the default
        require(params.errorPages.unauthorized)(app, req, res)

        // * this serves whatever is on this route
        // res.redirect('/403')
      } else {
        next()
      }
    ...
    if (params.csrfProtection === 'full') {
      ...
      // attach the middleware to the app so it can be applied to specific routes
