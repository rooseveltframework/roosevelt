...
module.exports = app => {
  const express = app.get('express') // the app's own express, which roosevelt.js loaded from the app
  ...
  function registerViewEngine (paramValue) {
    ...
    try {
      ...
      viewModule = requireFromApp(app.get('appDir'), viewEngine, require) // the view engine is the app's to install, so it is loaded from the app
