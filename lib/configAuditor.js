...
(function configAudit () {
  ...
  defaultConfigKeys.forEach(function (defaultParam) {
    if (!defaultConfig[defaultParam].optional) {
      if (userConfig[defaultParam] === undefined) {
        console.error('⚠️ ', `Missing param ${defaultParam}!`.red.bold)
        errors = true
      } else if (defaultConfig[defaultParam].value !== (undefined || []) && defaultConfig[defaultParam].value instanceof Object) {
        if (checkParamObject(userConfig[defaultParam], defaultConfig[defaultParam].value, defaultParam)) {
          errors = true
        }
      }
    }
