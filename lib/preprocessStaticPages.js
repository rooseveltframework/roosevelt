...
module.exports = async (app, pagesToRender) => {
  ...
  // an empty list means the edit was to something no page is built from, such as a stylesheet, so there is nothing to render
  if (pagesToRender && !pagesToRender.length) return
  ...
  const staticPagesSources = fs.existsSync(htmlPath) ? await walk(htmlPath) : []
  ...
  if (fs.existsSync(htmlPath)) {
    for (const file of await walk(htmlPath)) {
      // filter out irrelevant files
      if (isCandidate(app, file, gitignoreFiles)) {
        try {
          const baseFile = file.replace(app.get('params').html.sourcePath + path.sep, '')
          if (pagesToRender && !pagesToRender.includes(baseFile)) continue // this rebuild is only for the pages the edited files affect
          if (!isPage(app, file)) continue
          ...
          if (fs.pathExistsSync(modelFile)) {
            ...
            // node keeps what it requires, so a model edited since the last build would otherwise keep handing back the values it had then
            try {
              delete require.cache[require.resolve(modelFile)]
            } catch {
          ...
          // express puts its `view cache` setting on the render options alongside the locals, and view engines read it from there it is always off here: this build can run again while the app is up, and a cache that outlived one build would hand back the template as it was the first time rather than as it is now
          //
          // the express engine contract is `engine(path, options, callback)`, which is all that is relied on here, so any engine express itself would accept can build a page
          let newHtml = await promisify(renderer)(baseFile, { ...model, cache: false })

          // construct destination for rendered html
          const outpath = outputPath(app, file, message => logger.warn(message))
          let content
...
// the pages the generator builds: each file it renders, and the file it writes the page to
//
// this is for anything else that needs to know the site's pages, such as the sitemap, and works them out with the same rules the generator uses, so the two cannot disagree about which files are pages or where they end up
module.exports.pages = async app => {
  const params = app.get('params')
  const htmlPath = app.get('htmlPath')
  if (!params.html.sourcePath || !fs.existsSync(htmlPath)) return []
  const gitignoreFiles = gitignoreScanner(path.join(app.get('appDir'), '.gitignore'))
  const pages = []
  for (const file of await walk(htmlPath)) {
    if (!isCandidate(app, file, gitignoreFiles) || !isPage(app, file)) continue
    const extension = path.extname(file).slice(1)
    if (!app.get('view: ' + extension)) continue // no view engine can render it, which the generator reports when it builds
    pages.push({ file, output: outputPath(app, file, () => {}) })
  }
  return pages
}

// a file in the pages folder that is worth looking at at all: not a folder, and not something the app would not commit
function isCandidate (app, file, gitignoreFiles) {
  return file !== '.' && file !== '..' && !gitignoreFiles.includes(path.basename(file)) && !gitignoreFiles.includes(file) && !fs.lstatSync(file).isDirectory()
}

// a candidate that is a page to build: allowed by the allowlist and blocklist, not a model file, and not opted out with a roosevelt-blocklist comment on its first line
function isPage (app, file) {
  const { allowlist, blocklist } = app.get('params').html
  const baseFile = file.replace(app.get('params').html.sourcePath + path.sep, '')

  // skip this file if it's not on the allowlist but only if an allowlist exists
  //
  // also skip it if it's on the blocklist
  if ((allowlist && allowlist.length > 0 && !wildcardMatch(baseFile, allowlist)) || (blocklist && blocklist.length > 0 && wildcardMatch(baseFile, blocklist))) return false

  const extension = path.extname(file).slice(1)
  if (extension === 'js' || extension === 'json') return false

  const fileComment = fs.readFileSync(file, 'utf8').trim().split('\n')[0]
  if (fileComment.includes('roosevelt-blocklist')) return false
  return true
}

// where the page built from a file is written, which the folderPerPage param decides
//
// warn is told when folderPerPage is set to something it cannot apply, in which case the page is written where it would be without it
function outputPath (app, file, warn) {
  const params = app.get('params')
  const appName = app.get('appName')
  const htmlPath = app.get('htmlPath')
  const htmlRenderedOutput = app.get('htmlRenderedOutput')
  ...
  if (params.html.folderPerPage) {
    if (params.html.folderPerPage === true) outpath = path.join(htmlRenderedOutput, dir.replace(htmlPath, ''), `${name}/${name}.html`)
      ...
      if (params.html.folderPerPage.includes('/')) warn(`${appName} failed to apply \`folderPerPage\` param because it is an improperly formatted string. Please ensure that it is set correctly.`)
      else {
    ...
    } else warn(`${appName} failed to apply \`folderPerPage\` param because it is neither a boolean nor a string. Please ensure that it is set correctly.`)
  }
  return outpath
}
