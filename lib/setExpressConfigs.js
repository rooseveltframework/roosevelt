...
module.exports = function (app) {
  ...
  if (params.expressSession) {
    ...
    if (params.store.instance) {
      store = params.store.instance
    } else if (params.store.preset === 'default') {
      ...
      store = new MemoryStore({ ...params.store.presetOptions })
    } else if (params.store.preset === 'express-session-default') {
      store = null
    }
    ...
    ;(async () => {
      ...
      if (params.csrf === 'manual') app.set('csrfProtection', csrf({ middleware: 'session' })) // choose which routes are protected
      else if (params.csrf === 'full') app.use(csrf({ middleware: 'session' })) // all routes protected
    })()
  }
