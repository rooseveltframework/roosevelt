...
module.exports = function (app, callback) {
  ...
  if (!fsr.fileExists(jsBundledOutput)) {
    if (fsr.ensureDirSync(jsBundledOutput)) {
      logger.info('📁', `${appName} making new directory ${jsBundledOutput}`.yellow)
    }
  }
  ...
  if (params.js.bundler.expose && !fsr.fileExists(bundleBuildDir)) {
    if (fsr.ensureDirSync(bundleBuildDir)) {
      logger.info('📁', `${appName} making new directory ${bundleBuildDir}`.yellow)
    }
  }
      ...
      promises.push(
        new Promise((resolve, reject) => {
          ...
          browserify(bundle.files, bundle.params).bundle((err, jsCode) => {
            if (err) {
              logger.error(`${appName} failed to write new JS file ${path.join(jsBundledOutput, bundle.outputFile)} due to syntax errors in the source JavaScript`)
              reject(err)
              return
            } else {
              // write bundled js file to js bundled output
              if (fsr.writeFileSync(path.join(jsBundledOutput, bundle.outputFile), jsCode, 'utf8')) {
                logger.info('📝', `${appName} writing new JS file ${path.join(jsBundledOutput, bundle.outputFile)}`.green)
              }
              ...
              if (params.js.bundler.expose) {
                if (fsr.writeFileSync(path.join(bundleBuildDir, bundle.outputFile), jsCode, 'utf8')) {
                  logger.info('📝', `${appName} writing new JS file ${path.join(bundleBuildDir, bundle.outputFile)}`.green)
                }
              }
            }
            resolve()
