...
module.exports = app => {
  ...
  // make statics folder if it doesn't exist
  if (!fsr.fileExists(params.staticsRoot)) {
    if (fsr.ensureDirSync(params.staticsRoot)) {
      logger.info('📁', `${appName} making new directory ${params.staticsRoot}`.yellow)
    }
  }
  ...
  if (params.generateFolderStructure) {
    params.staticsSymlinksToPublic.forEach(pubStatic => {
      ...
      const staticTarget = path.join(params.staticsRoot, (pubStatic[1] || pubStatic[0]).trim())
      ...
      if (publicFolderDirs) {
        ...
        if (routers.public) {
          publicFolderDirs.forEach(publicDir => {
            ...
            // public wasn't assigned to a route so assign it to "/" + directory name
            if (!publicAssigned) {
