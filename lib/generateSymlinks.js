...
module.exports = app => {
  ...
  if (params.makeBuildArtifacts) {
    ...
    // the test files live in this app's own build folder and carry the process id, because two apps started from the same working directory used to write to the same path and delete it out from under each other, which made one of them report that symlinks do not work
    const source = path.join(params.buildFolder, `symlinkTestSource-${process.pid}`)
    const dest = path.join(params.buildFolder, `symlinkTest-${process.pid}`)
    try {
      fs.outputFileSync(source, '')
    ...
    startupNotice('noBuildArtifacts', '💡', `${appName} will not generate build artifacts like symlinks, compiled/bundled CSS/JS files, etc because the makeBuildArtifacts parameter is set to false. See Roosevelt API docs for more information about the makeBuildArtifacts parameter.`)
  }
}
