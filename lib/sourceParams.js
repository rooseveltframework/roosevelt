...
module.exports = (params, app, appSchema) => {
  ...
  const config = {
    ...
    transform: (params, flags) => {
      ...
      // handle webpack=verbose, --wp, -p
      if (flags.webpack === 'verbose' || flags.wp || flags.p) {
        params.logging.methods.verbose = true
        params.logging.methods.verboseFile = false
      }

      // handle verbose-file, --wf . -f
      if (flags.webpack === 'verbose-file' || flags.wf || flags.f) {
        params.logging.methods.verbose = true
        params.logging.methods.verboseFile = true
      }
