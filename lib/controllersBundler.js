...
module.exports = async app => {
  ...
  const allControllersFiles = (await walk(controllersPath)).filter(file => !finalBlocklist.has(file))
  ...
  for (const bundleName of Object.keys(bundles)) {
    try {
      ...
      const bundleFiles = matchingFiles(bundleGlob, controllersPath, finalBlocklist)
      const writePath = path.join(app.get('clientControllersBundledOutput'), bundleName)

      // write controllers bundle only if it contains new/edited files
      const bundleSources = bundleFiles.map(file => path.join(controllersPath, file))
      if (buildCache.isFresh(writePath, bundleSources)) continue
...
`
      ...
      buildCache.record(writePath, { outputs: writePath, sources: bundleSources })
    } catch (err) {
