...
module.exports = function (app, callback) {
  ...
  function applyValidatorMiddleware () {
    ...
    exception = params.htmlValidator.exceptions[0]
    ...
    app.use(tamper((req, res) => {
      ...
      if (req.headers[exception.requestHeader]) {
        res.set(exception.requestHeader, true)
      }

      if (res.statusCode === 200 && res.getHeader('Content-Type') && res.getHeader('Content-Type').includes('text/html') && !res.getHeader(exception.requestHeader)) {
        return body =>
          new Promise((resolve) => {
            options.data = body
            options.format = 'json'
