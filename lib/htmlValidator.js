  ...
  function applyValidatorMiddleware () {
    ...
    app.use(tamper((req, res) => {
      ...
      function validatorExceptions () {
        ...
        for (let exception of headerException) {
    ...
    if (params.htmlValidator.separateProcess.enable && params.htmlValidator.separateProcess.autoKiller) {
      ...
      if (test === true) {
        ...
        fkill(PID, { force: true }).then(() => {
          ...
          let autokiller = spawn('node', [`${path.join(__dirname, 'scripts/autoKillValidator.js')}`, `${params.port}`, `${params.htmlValidator.separateProcess.autoKillerTimeout}`, params.logging.methods.verbose], { detached: true, stdio: 'inherit', shell: false, windowsHide: true })
          autokiller.unref()
          cb()
        }, () => {
