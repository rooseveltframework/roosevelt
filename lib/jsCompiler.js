...
module.exports = function(app, callback) {
  ...
  if (typeof preprocessorModule.parse === 'function') {
    preprocessorArgs = utils.getArguments(preprocessorModule.parse);

    if ((preprocessorArgs.length !== 2) || (preprocessorArgs[0] !== 'app') || (preprocessorArgs[1] !== 'fileName')) {
      console.error(`❌  selected javascript compiler module ${preprocessor} out of date or incompatible with Roosevelt.`.red.bold);
      process.exit();
    }
  }
  else {
