...
describe('Roosevelt Config Auditor Test', function () {
  ...
  it('should only check direct dependencies when installing prod only', function (done) {
    // set env.INIT_CWD to the correct location
    process.env.INIT_CWD = appDir
    ...
    // set up package.json
    packageJSONSource.dependencies = {}
    packageJSONSource.scripts.postinstall = 'node ../../../lib/scripts/configAuditor.js'

    // bool var to hold whether or not the right logs for checking dependencies was the correct output
    let isCheckingProd = false

    // generate the package.json file
    fs.ensureDirSync(appDir)
    fs.writeFileSync(path.join(appDir, 'package.json'), JSON.stringify(packageJSONSource))

    const prodInstall = spawnSync(npmName, ['install', '--only=prod'], { cwd: appDir, stdio: 'pipe' })

    if (prodInstall.output.toString().includes('Production install detected. Checking direct dependencies.')) {
      isCheckingProd = true
    }

    assert.strictEqual(isCheckingProd, true, 'config auditor is not checking just direct dependencies')
    done()
  })

  it('should check all dependencies on npm install', function (done) {
    // set env.INIT_CWD to the correct location
    process.env.INIT_CWD = appDir
    ...
    const prodInstall = spawnSync(npmName, ['install'], { cwd: appDir, stdio: 'pipe' })

    if (prodInstall.output.toString().includes('Checking dependencies and dev dependencies.')) {
      isCheckingAll = true
    }

    assert.strictEqual(isCheckingAll, true, 'config auditor is not checking all dependencies when it should be')
    done()
  })
})
