...
module.exports = function (app, params) {
  ...
  const schema = {
    ...
    logging: {
      default: checkParam(defaultLogging, 'logging')
    },
    minify: {
    ...
    publicFolder: {
      default: checkParam('public', 'publicFolder')
    },
    favicon: {
    ...
    clientViews: {
      whitelist: {
        default: checkParam({}, 'whitelist', ['clientViews'])
      },
      blacklist: {
        default: checkParam([], 'blacklist', ['clientViews'])
      },
      output: {
        default: checkParam('.build/templates', 'output', ['clientViews'])
      },
      exposeAll: {
        default: checkParam(false, 'exposeAll', ['clientViews'])
      },
      defaultBundle: {
        default: checkParam('bundle.js', 'defaultBundle', ['clientViews'])
      },
      minify: {
        default: checkParam(true, 'minify', ['clientViews'])
      },
      minifyOptions: {
        default: checkParam({}, 'minifyOptions', ['clientViews'])
      }
    },
    onServerInit: {
    ...
    onClientViewsProcess: {
      default: params.onClientViewsProcess || {}
    },
    jsCompiler: {
  ...
  ;(function paramVariables (paramSet, paramSource) {
    ...
    if (paramSet === Object(paramSet)) {
      Object.keys(paramSet).forEach(paramKey => {
        ...
        } else if (Array.isArray(param)) { // recurse if param value is array
          paramVariables(param, paramSource(paramKey))
        } else if (typeof param === 'string') { // check for templating when param is a string
          paramSource[paramKey] = template(param, params, { partial: true })
        }
      })
    } else if (Array.isArray(paramSet)) {
      paramSet.forEach((item, i) => {
        // recurse if param value is an object
        if (item === Object(item)) {
          paramVariables(item, paramSource[i])
        } else if (Array.isArray(item)) { // recurse if param value is array
          paramVariables(item, paramSource(i))
        } else if (typeof param === 'string') { // check for templating when param is a string
          paramSource[i] = template(item, params, { partial: true })
        }
      })
    }
  })(params)

  // configure and store logger in express variable
  ...
  if (!params.generateFolderStructure && pkgNotFound) {
    logger.warn(`Roosevelt initialized and configured solely via constructor! (${pkgErr.toString().includes('rooseveltConfig') ? 'No rooseveltConfig found in package.json' : 'No package.json found'})`)
  ...
  // parse cli args
  const flags = require('./sourceFlags')(app, params.enableCLIFlags)
